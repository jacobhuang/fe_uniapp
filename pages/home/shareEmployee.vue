	...
	<view>
		...
		<!-- 	<swiper class="screen-swiper square-dot"  :indicator-dots="true" :circular="true"
			...
			<view class="cu-bar bg-white solid-bottom" >
				<view class="action">
					<text class='cuIcon-title text-blue'></text>报表分析
				</view>
			</view>
			...
			<view class="cu-bar bg-white solid-bottom margin-top"  >
				<view class="action">
					 <text class='cuIcon-title text-yellow'></text>操作中心
				</view>
			</view>
			<view class="cu-list grid col-3 text-sm">
			...
			<view class="cu-bar bg-white solid-bottom margin-top" >
				<view class="action">
					 <text class='cuIcon-title text-yellow'></text>公共设置
				</view>
			</view>
			<view class="cu-list grid col-3 text-sm">
				<view class="cu-item"  v-for="(item,index) in baseList" :key="index" @tap="goPage(item.page)">
					<view class="padding text-center">
						<image :src="item.icon" style="width:28px;height:28px;"/>
						<view class="margin-top-xs">{{item.title}}</view>
					</view>
...
<script>
	import { us,os,base } from '@/common/util/shareEmployee.js'
