  ...
  <view>
    <view>
		<cu-custom bgColor="bg-black" :isBack="true">
			<block slot="content">专属管家排行</block>
		</cu-custom>
		
	</view>
    <view v-if="current == 0" class="list">
      <!-- <u-header-count
        :totalMerchant="data.summary.total_merchants"
      ></u-header-count> -->
	   <u-header-count :merchant-label="labelName1" :rechargeAmountLabel="labelName2" :rechargeBalanceLabel="labelName3"></u-header-count>
	
      <scroll-view class="list-content" scroll-y style="display:none;">
        <view
          class="list-item"
          v-for="(item, index) in data.summary.list"
          :key="index"
        >
      ...
      <view class="list card">
        <!-- 在此添加可滚动列表，显示字段及接口名请参见要求 -->
          ...
          <scroll-view class="list-content" scroll-y style="display:none;">
            <view
              class="list-item"
              v-for="(item, index) in data.record.list"
              :key="index"
            >
              <view class="item-index">{{ index + 1 }}</view>
              <view class="item-info">
