<template>
	<view>
		...
		<view v-if="current == 0" class="record">
			<view class="card">
	...
	export default {
		...
		data() {
			return {
				...
				endDate: new Date(),
				isLoading: true,
				urlSummary: '/sharerShop/report/balanceSummary?page=1&pageSize=10',
				urlRecord: '/sharerShop/report/balanceRecords?page=1&pageSize=10',
				data: {
					summary: {
						total_merchants: 0,
		...
		methods: {
			...mapActions(["balanceSummary", "balanceRecords"]),
			loadMore() {
			...
			loadBalanceRecords(url, params) {
				this.$http.get(url, {
						...
						if (res.data.success) {
							this.data.record.list.push(...res.data.result.balanceRecordDataList);
						} else {
			...
			loadBalanceSummary(url, params) {
				// this.balanceRecords(params)
				// 	.then((res) => {
				this.$http.get(url, {
						params: params
					}).then(res => {
						this.isLoading = false;
						if (res.data.success) {
							this.data.summary.list.push(...res.data.result.balanceSummaryDataList);
						} else {
							this.$tip.alert(res.data.message);
						}
					})
			...
			loadData(index) {
				switch (index) {
					case 0:
						...
						this.loadBalanceSummary(this.urlSummary, paramsSummary);
						break;
					case 1:
						var paramsRecord = {
							page: 1,
						...
						this.loadBalanceRecords(this.urlRecord, paramsRecord);
