	...
	<view >
		<view>
			<cu-custom bgColor="bg-black" :isBack="true">
				<block slot="content">共享掌柜管理</block>
			</cu-custom>
			<view class="bg-black">
			<u-tabs
				class="tabs-style"
			...
			<view class="scheme-list" >
				<view v-if="current == 0">
					...
					<view class="card-wrapper" v-for="(item, index) in schemeList" :key="index">
						...
						<view class="card-tag-right" v-if="item.giftType">{{ item.giftType }}</view>
						<view class="card-content">
							...
							<view class="card-line">
								<view class="profit-rate">{{ item.giftAmount }}</view>
								<view class="profit-rate">{{ item.giftPlan }}</view>
							</view>
                ...
                <view v-if="current == 2" style="height:100vh;">
					...
					<view class="setting-content">
						<view class="input-form">
								待激活掌柜是指：<view class="input-wrapper">
									<input type="text" placeholder="" />
								</view>天（含以上）未到店消费或者<view class="input-wrapper">
									<input type="text" placeholder="" />
								</view>天（含以上）未拓展客源
