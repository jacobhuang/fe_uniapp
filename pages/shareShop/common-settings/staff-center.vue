        ...
        <view v-if="current == 0">
          ...
          >
            ...
            <view class="card-footer">
              <view class="btn-wrap">
                <view class="btn-delete" @click="deleteScheme(index)">{{
                  btnText.delete
                }}</view>
                <view class="btn-use" @click="useScheme(index)">{{
                  btnText.use
                }}</view>
              </view>
            </view>
          ...
          >
            <view class="card-content">
              <view class="card-line">
                ...
                <view class="dividing-ratio">{{ item.createTime }}</view>
              </view>
            </view>
            <view class="card-footer">
              <view class="btn-wrap">
                <view class="btn-edit" @click="editScheme(index)">{{
                  btnText.edit
                }}</view>
                <view class="btn-use" @click="useScheme(index)">{{
                  btnText.use
...
export default {
  ...
  methods: {
    ...
    initRoleList() {
      // api.fetchData 获取数据的方法（这里采用mock数据作为示例）
      this.roleList = [...Array(10)].map((v, i) => ({
        name: "服务员",
    ...
    editScheme(index) {
      ...
      // 跳转到编辑页面，并传入方案数据
      uni.navigateTo({
        url: `/pages/home/costEstimate/editScheme/editScheme?scheme=${JSON.stringify(
          scheme
        )}&index=${index}`,
      });
    },
...
<style scoped>
view {
  --UI-BACKGROUND-BLACK: #000000;
  --UI-BORDER-GREY: #cccccc;
}
...
.input-wrapper {
  border: 1px solid var(--UI-BORDER-GREY);
  border-radius: 4px;
  padding: 4px;
  margin-top: 8px;
  margin: 0px 10px;
